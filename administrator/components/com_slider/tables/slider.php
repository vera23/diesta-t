...
class SliderTableslider extends JTable
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'SliderTableslider', array('typeAlias' => 'com_slider.slider'));
		parent::__construct('slider', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		// Support for checkbox field: active
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_slider.slider.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_slider/access.xml',
				"/access/section[@name='slider']/"
			);
			$default_actions = JAccess::getAssetRules('com_slider.slider.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		if ($files['image'][0]['size'] > 0)
		{
			...
			$oldFiles = SliderHelpersSlider::getFiles($this->id, $this->_tbl, 'image');

			foreach ($oldFiles as $f)
			{
				$oldFile = JPATH_ROOT . '/../images/slider/' . $f;
			...
			foreach ($files['image'] as $singleFile )
			{
				...
				else
				{
					...
					$uploadPath = JPATH_ROOT . '/../images/slider/' . $filename;
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_slider');
	...
	public function delete($pk = null)
	{
		...
		if ($result)
		{
			...
			{
				JFile::delete(JPATH_ROOT . '/../images/slider/' . $imageFile);
			}
		}
